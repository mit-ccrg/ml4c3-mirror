...
def update_tmaps(tmap_name: str, tmaps: Dict[str, TensorMap]) -> Dict[str, TensorMap]:
    ...
    # Base tmaps: STS
    from ml4cvd.tensormap.tensor_maps_sts import tmaps as tmaps_sts  # isort:skip
    tmaps.update(tmaps_sts)
    if tmap_name in tmaps:
        return tmaps

    # Base tmaps: ECG labels
    from ml4cvd.tensormap.tensor_maps_ecg_labels import tmaps as tmaps_ecg_labels  # isort:skip
    tmaps.update(tmaps_ecg_labels)
    if tmap_name in tmaps:
        return tmaps

    # Base tmaps: ECG voltage
    from ml4cvd.tensormap.tensor_map_updaters import update_tmaps_ecg_voltage  # isort:skip
    tmaps = update_tmaps_ecg_voltage(tmap_name=tmap_name, tmaps=tmaps)
    if tmap_name in tmaps:
        return tmaps

    # Modify: sts match ecg tensor dimensions
    from ml4cvd.tensormap.tensor_map_updaters import update_tmaps_sts_match_ecg # isort: skip
    tmaps = update_tmaps_sts_match_ecg(tmap_name=tmap_name, tmaps=tmaps)
    if tmap_name in tmaps:
        return tmaps

    # Modify: weighted loss
    from ml4cvd.tensormap.tensor_map_updaters import update_tmaps_weighted_loss  # isort:skip
    tmaps = update_tmaps_weighted_loss(tmap_name=tmap_name, tmaps=tmaps)
    if tmap_name in tmaps:
        return tmaps

    # Modify: STS window (e.g. preop)
    from ml4cvd.tensormap.tensor_map_updaters import update_tmaps_sts_window  # isort:skip
    tmaps = update_tmaps_sts_window(tmap_name=tmap_name, tmaps=tmaps)
    if tmap_name in tmaps:
        return tmaps

    # Modify: time series
    from ml4cvd.tensormap.tensor_map_updaters import update_tmaps_time_series  # isort:skip
    tmaps = update_tmaps_time_series(tmap_name=tmap_name, tmaps=tmaps)
    if tmap_name in tmaps:
        return tmaps

    # Modify: load predictions
    from ml4cvd.tensormap.tensor_map_updaters import update_tmaps_model_predictions  # isort:skip
