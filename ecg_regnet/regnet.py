...
class GroupConv(Layer):
    ...
    def __init__(self, filters: int, groups: int, kernel_size: int, stride: int):
        ...
        self.convs = [
            Conv1D(
                filters=split, kernel_size=kernel_size, strides=stride,
    ...
    def _filters_per_conv(total_filters: int, groups: int) -> List[int]:
        ...
        return (
                extras * [filters_per_section + 1]
                + (groups - extras) * [filters_per_section]
        )
...
class Stem(Layer):
    def __init__(self, width: int, kernel_size: int, activation: Activation):
        ...
        self.conv = Conv1D(
            filters=width, strides=2, padding='same', kernel_size=kernel_size,
...
class SqueezeExcite(Layer):
    ...
    def build(self, input_shape):
        ...
        self.dense_1 = Dense(
            channels // self.ratio, activation='relu',
            kernel_initializer='he_normal', use_bias=False,
        )
        self.dense_2 = Dense(
...
class Block(Layer):
    ...
    def __init__(
            self, width: int, groups: int, stride: int, kernel_size: int,
            activation: Callable,
    ):
        super(Block, self).__init__()
        if stride > 1:
            self.shortcut_conv = Conv1D(
                filters=width, strides=stride, padding='same', kernel_size=kernel_size,
                kernel_initializer='he_normal', use_bias=False,
            )
        else:
        ...
        self.conv_1 = Conv1D(
            filters=width, strides=1, padding='same', kernel_size=kernel_size,
            kernel_initializer='he_normal', use_bias=False,
        )
        self.group_conv = GroupConv(filters=width, groups=groups, stride=stride, kernel_size=kernel_size)
        self.conv_3 = Conv1D(
            filters=width, strides=1, padding='same', kernel_size=kernel_size,
            kernel_initializer='he_normal', use_bias=False,
        )
...
class Stage(Layer):
    def __init__(
            ...
            group_size: int, kernel_size: int,
            activation: Callable,
    ):
    ...
    def build(self, input_shape):
        ...
        self.blocks = [
            Block(
                width=self.width, groups=channels // self.group_size,
                stride=2, kernel_size=self.kernel_size, activation=self.activation,
            )
        ]
        for _ in range(self.depth - 1):
            self.blocks.append(Block(
                width=self.width, groups=channels // self.group_size,
                stride=1, kernel_size=self.kernel_size, activation=self.activation,
            ))
...
class RegNetYBody(Layer):
    def __init__(
        ...
        self.stem = Stem(
            initial_width, kernel_size * 3, tf.nn.swish,
        )
        widths = self.find_widths(
            depth, initial_width, width_growth_rate, width_quantization,
        )
        widths, depths = np.unique(widths, return_counts=True)
        self.stages = [
            Stage(
                width=width, depth=depth, group_size=group_size,
                kernel_size=kernel_size, activation=tf.nn.swish,
            )
    ...
    @staticmethod
    def find_widths(
            depth: int, initial_width: int,
            width_growth_rate: float, width_quantization: float,
    ):
...
class ML4C3Regnet(Model):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        return {
            name: dense(embed)
            for name, dense in self.denses.items()
        }
...
def _test_regnet():
    ...
    reg = ML4C3Regnet(
        depth=10, initial_width=32, width_growth_rate=3, width_quantization=2,
        group_size=3, kernel_size=3,
        input_name='ecg', output_name_to_shape=output_name_to_shape,
    )
    batch_size = 3
