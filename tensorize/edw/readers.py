...
class CrossReferencer:
    ...
    def __init__(
        ...
        self.edw_dir = edw
        self.adt = adt
        self.crossref: Dict[str, List[str]] = {}
    ...
    def get_xref_files(
        ...
        """
        Get the cross-reference between ADT and EDW files.

        The output dictionary will have the format:

        {"MRN1": {"visitID", "visitID2", ...}
         "MRN2": ...,
         ...}
        ...
        :return: <dict> a dictionary with the MRNs and visit IDs.
        """
        self.crossref = {}
        ...
        if not overwrite_hd5 and tensors and os.path.isdir(tensors):
            ...
            adt_df = adt_df[adt_df["MRN"].isin(existing_mrns)]
    ...
    def add_edw_elements(self, adt_df):
        # Add elements from EDW folders
        for _, row in adt_df.iterrows():
            mrn = str(int(row.MRN))
            ...
            if os.path.isdir(os.path.join(self.edw_dir, mrn, csn)):
                if mrn not in self.crossref:
                    ...
                    self.crossref[mrn].append(csn)
            else:
                logging.info(f"MRN: {mrn}, CSN: {csn}, not found in {self.edw_dir}.")
