...
def remove_unfinished_hd5(folder, no_bm):
    hd5_files_path = [
    ...
    for hd5_file_path in hd5_files_path:
        ...
        try:
            with h5py.File(hd5_file_path, "r") as hd5_file:
                finished = _check_if_finished(hd5_file, no_bm)
            if not finished:
...
def _check_if_finished(hd5_file, no_bm):
    try:
        if no_bm:
            for bm_visit in hd5_file["bedmaster"].keys():
                if len(hd5_file[f"bedmaster/{bm_visit}"].keys()) == 0:
                    return False
        return (
